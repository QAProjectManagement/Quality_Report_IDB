...
def prepare_ffr_trend_data(
    ...
    if x_col == "Month":
        valid_x = [month for month in month_order if month in grouped[x_col].unique()]
        grouped = grouped[grouped[x_col].isin(valid_x)]
        grouped["Month"] = pd.Categorical(grouped["Month"], categories=valid_x, ordered=True)
...
def prepare_percentage_data(
    ...
    pivot_df = pivot_df.replace(0, 0.01)
